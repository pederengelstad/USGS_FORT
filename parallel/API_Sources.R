...
par_api_data <- function(species_list = NULL, sources=c('gbif','bison','inat','ecoengine','eddmaps'), edd.spdf
  ...
  if(length(v[which(v%in%sources)]) > 0) {
    ...
    cat('Searching GBIF/BISON/iNaturalist/EcoEngine for occurrence records \n')
    ...
    if(sum(sapply(spocc_df$gbif$data, NROW)) > 0){
      ...
      if('coordinateUncertaintyInMeters' %in% names(gbif_df)){
